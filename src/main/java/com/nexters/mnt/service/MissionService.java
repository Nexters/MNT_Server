...
//        firebaseUtil.sendFcmUserToUser(user.getFcmToken(), "마니또!", "테스트~!!");
    ...
    public ApiResponse<List<UserMissionResponse>> getUserMission(String userId, Long roomId){
        return new ApiResponse<>(userMissionRepository.findByUserIdAndRoomId(userId, roomId).stream()
                .map(UserMission::convertToUserMissionResponse).collect(Collectors.toList()),ApiStatus.Ok);
    }
    ...
    public ApiResponse<List<UserMissionResponse>> getUserReceiveMission(String userId, Long roomId) {
        ...
        return new ApiResponse<>(userMissionRepository.findByUserIdAndRoomId(manittoId, roomId).stream()
                .map(UserMission::convertToUserMissionResponse).collect(Collectors.toList()),ApiStatus.Ok);
    }
}
